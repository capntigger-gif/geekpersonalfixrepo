...
export class MangaProgressImplementation implements MangaProgressProviding {
  ...
  async processChapterReadActionQueue(
    ...
    for (const [mangaId, mangaActions] of Object.entries(byManga)) {
      try {
        // Mark the series as read when a chapter is completed. League of Comic Geeks'
        // Read List is the tracker state; Collection means owned, not read.
        await addToList(mangaId, LIST_IDS.READ_LIST);
        successfulItems.push(...mangaActions.map((a) => a.id));
        console.log(`${logPrefix} marked ${mangaId} as read`);
      } catch (e) {
